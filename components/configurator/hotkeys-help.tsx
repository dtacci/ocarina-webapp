...
const SHORTCUTS: { keys: string[]; label: string }[] = [
  { keys: ["1", "2", "…", "9", "0", "-", "="], label: "Focus button 1–12" },
  { keys: ["C", "D", "E", "F", "G", "A", "B"], label: "Assign note to focused button" },
  { keys: ["⇧", "+", "letter"], label: "Sharp (C# / D# / F# / G# / A#)" },
  { keys: ["R", "Backspace"], label: "Reset focused button to default" },
  { keys: ["Esc"], label: "Clear focus" },
  { keys: ["?"], label: "Open this cheat sheet" },
];

export function HotkeysHelp({ open, onOpenChange }: Props) {
  return (
    <Popover open={open} onOpenChange={onOpenChange}>
      ...
      <PopoverContent className="w-[min(20rem,calc(100vw-2rem))] p-3" align="end">
        ...
        <p className="mt-3 text-[10px] text-muted-foreground/70">
          Letters work case-insensitively. Sharps use Shift.
        </p>
      </PopoverContent>
    </Popover>
  );
