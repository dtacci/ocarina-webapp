...
interface DrumMachineProps {
  ...
  deviceName?: string;
  compact?: boolean;
  /** A groove generated by the "sounds-like" workshop to load into the grid. */
  incomingGroove?: IncomingGroove | null;
}

export function DrumMachine({ deviceId, compact, incomingGroove }: DrumMachineProps) {
  ...
  const [prevGroove, setPrevGroove] = useState<IncomingGroove | null>(null);
  ...
  // Apply a groove handed in from the "sounds-like" workshop. Done during render
  // (React's "adjust state when a prop changes" pattern) rather than an effect,
  // guarded so it runs once per distinct groove — the prop identity changes on
  // each send.
  if (incomingGroove && incomingGroove !== prevGroove) {
    setPrevGroove(incomingGroove);
    const manifest =
      kits.find((k) => k.id === incomingGroove.kitId) ??
      BUILTIN_KITS.find((k) => k.id === incomingGroove.kitId);
    if (manifest) setKit(manifest);
    pattern.loadPattern(incomingGroove.pattern);
    setLocalBpm(incomingGroove.bpm);
  }
